/* Intel PT / ARM CoreSight full hardware trace.
 * Decodes aux buffer → complete kernel PC trace.
 */
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
int intel_pt_start(struct vock_hw_ctx *ctx, pid_t pid)
{
	...
	if (ctx->perf_fd < 0) {
		perror("intel_pt: perf_event_open");
		return -1;
	}
...
int intel_pt_decode(struct vock_hw_ctx *ctx, const char *vmlinux)
{
	...
	if (len > ctx->aux_size)
		len = ctx->aux_size;
	if (len == 0) {
		fprintf(stderr, "[vock] intel_pt: no data captured\n");
		return -1;
	}
	...
	if (!f) {
		perror("intel_pt: fopen kerncov.log");
		return -1;
	}
	...
	if (vmlinux) {
		...
		if (pt_decoder_init(&dec, vmlinux, data, len, f) == 0) {
			...
			fprintf(stderr, "[vock] intel_pt: vmlinux load failed, TIP-only mode\n");
			goto tip_only;
		}
	} else {
tip_only:;
	...
	fclose(f);
	fprintf(stderr, "[vock] intel_pt: %d kernel PCs → kerncov.log\n", pc_count);
	return 0;
}
